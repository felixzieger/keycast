...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!(target: "signing_daemon", "Signing daemon for authorization {:?} started", auth_id);
    tracing::info!(target: "signing_daemon", "Bunker signing URI: {:?}", signer.bunker_uri());
...
impl<T: AuthorizationValidations> NostrConnectSignerActions for SignerActions<T> {
    fn approve(&self, request: &Request) -> bool {
        tracing::debug!("Evaluating request: {:?}", request);

        // Check to see if the authorization is expired
        match self.signer_daemon.authorization.expired() {
            Ok(true) => {
                tracing::error!(target: "signing_daemon", "Authorization expired");
                return false;
            }
            Ok(false) => (),
            Err(e) => {
                tracing::error!(target: "signing_daemon", "Error checking if authorization is expired: {:?}", e);
                return false;
            }
        }
        ...
        {
            Ok(true) => {
                tracing::error!(target: "signing_daemon", "Authorization fully redeemed");
                return false;
            }
            Ok(false) => (),
            Err(e) => {
                tracing::error!(target: "signing_daemon", "Error checking if authorization is fully redeemed: {:?}", e);
                return false;
            }
        }
        ...
        match request {
            Request::Connect { public_key, secret } => {
                tracing::info!(target: "signing_daemon", "Connect request received");
                return true;
            }
            Request::GetPublicKey => {
                tracing::info!(target: "signing_daemon", "Get public key request received");
                return true;
            }
            Request::SignEvent(event) => {
                tracing::info!(target: "signing_daemon", "Sign event request received");
                return true;
            }
            Request::GetRelays => {
                tracing::info!(target: "signing_daemon", "Get relays request received");
                return true;
            }
            Request::Nip04Encrypt { public_key, text } => {
                tracing::info!(target: "signing_daemon", "NIP04 encrypt request received");
                return true;
            }
            Request::Nip04Decrypt {
                ...
                tracing::info!(target: "signing_daemon", "NIP04 decrypt request received");
                return true;
            }
            Request::Nip44Encrypt { public_key, text } => {
                tracing::info!(target: "signing_daemon", "NIP44 encrypt request received");
                return true;
            }
            Request::Nip44Decrypt {
                ...
                tracing::info!(target: "signing_daemon", "NIP44 decrypt request received");
                return true;
            }
            Request::Ping => {
                tracing::info!(target: "signing_daemon", "Ping request received");
                return true;
            }
            _ => {
                tracing::error!(target: "signing_daemon", "Unsupported request: {:?}", request);
                return false;
            }
        }
    }
